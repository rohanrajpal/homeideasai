import { oauthGoogleJwtAuthorize } from "@/app/clientService";
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* <div className="text-sm text-amber-600 dark:text-amber-400 text-center">
        ⚠️ Google Sign In is temporarily unavailable due to provider issues. Our
        team is working on it. Please use regular login in the meantime.
      </div> */}
      <div className="flex items-center justify-center dark:bg-gray-700">
        <button
          onClick={async () => {
            const googleLoginUrl = await oauthGoogleJwtAuthorize();
            if (googleLoginUrl.data?.authorization_url) {
              toast({
                title: "Redirecting to Google login",
              });
              window.location.href = googleLoginUrl.data.authorization_url;
            }
          }}
