...
// Query: *[  _type == "post"  && defined(slug.current)  && (isFeatured != true || defined($category))  && select(defined($category) => $category in categories[]->slug.current, true)]|order(publishedAt desc)[$startIndex...$endIndex]{  title,  "slug": slug.current,  publishedAt,  excerpt,  mainImage,  author->{    name,    image,  },}
...
// Variable: POST_QUERY
// Query: *[  _type == "post"  && slug.current == $slug][0]{  publishedAt,  title,  mainImage,  excerpt,  body,  "headings": body[style in ["h2", "h3", "h4", "h5", "h6"]],  author->{    name,    image,  },  categories[]->{    title,    "slug": slug.current,  }}
